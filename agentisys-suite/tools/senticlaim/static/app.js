...
async function analyze() {
  ...
  try {
    const res = await fetch('/api/senticlaim/analyze', {
      method: 'POST',
...
sectorSelect.addEventListener('change', async () => {
  const res = await fetch('/api/senticlaim/sector', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function init() {
  const sectorsRes = await fetch('/api/senticlaim/sectors');
  ...
  const bootstrapRes = await fetch('/api/senticlaim/bootstrap');
